...
- `SceneRenderItem`, `RasterLayerRenderItem`, `SampledLayerRenderItem`, `SampledTileRenderData`, and `SceneLayerHitTestResult` are detached presentation products. Sampled tiles and layers expose constant-time geometry and immutable-product identity without reading pixels.
- `TransientRasterContribution`, `TransientRasterResolvedContribution`, `TransientSampledResolvedContribution`, and `TransientRasterTransformContribution` carry host-owned raster changes through normal scene presentation. Transform contributions optionally provide a `destination_attenuation_mask`; pixels outside that mask preserve the destination, and the premultiplied fragment is added after attenuation. Resolved contributions use `retain_until_durable` to distinguish committed handoff pixels from cancellable in-flight feedback. Active sampled contributions apply only to matching demand geometry. Committed sampled contributions remain view-scoped until view geometry changes or a later durable asset supersedes the handoff.
