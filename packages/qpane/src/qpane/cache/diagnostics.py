...
_CONSUMER_LABELS = {"tiles": "Tiles", "pyramids": "Pyramids"}
...
def cache_detail_provider(qpane: QPane) -> Iterable[DiagnosticRecord]:
    ...
    for consumer_id, state in sorted(consumers.items()):
        consumer_label = _CONSUMER_LABELS.get(
            consumer_id,
            consumer_id.replace("_", " ").title(),
        )
...
def _bundle_metrics(qpane: QPane, consumer_id: str) -> _CacheMetricsBundle:
    ...
    source_resolver = getattr(qpane, "_cache_metric_source", None)
    manager = source_resolver(consumer_id) if callable(source_resolver) else None
    if manager is None:
...
def _coerce_metrics(snapshot: object) -> _CacheMetricsBundle:
    ...
    values = {
        field_name: getattr(snapshot, field_name, None)
        for field_name in _CacheMetricsBundle.__dataclass_fields__
    }
    return _CacheMetricsBundle(**values)
