...
class CatalogHost(Protocol):
    """Widget operations required by catalog presentation and lifecycle."""

    settings: object
    original_image: QImage

    def width(self) -> int:
        """Return the widget width in logical pixels."""
        ...

    def windowHandle(self):
        """Return the current Qt window handle, when mounted."""
        ...

    def blank(self) -> None:
        """Display the host's blank presentation."""
        ...
...
class CatalogController:
    ...
    def setImagesByID(
        ...
        Side effects:
            Evicts derived products for removed or changed sources and optionally
        ...
        self._notify_resource_invalidated(
    ...
    def addImage(self, image_id: uuid.UUID, image: QImage, path: Path | None) -> None:
        ...
        Side effects:
            ...
            and tile caches when content changes or paths are displaced.
        """
        if image.isNull():
        ...
        self._evict_catalog_assets(mutation.cache_asset_keys_to_evict)
        self._notify_resource_invalidated(mutation.content_changed_ids)
    ...
    def removeImageByID(self, image_id: uuid.UUID) -> None:
        ...
        Side effects:
            Evicts derived source products, updates linked-view groups, and
            renders the placeholder when the catalog becomes empty.
        """
    ...
    def removeImagesByID(self, image_ids: Iterable[uuid.UUID]) -> tuple[uuid.UUID, ...]:
        ...
        Side effects:
            Evicts derived products for removed sources, updates link groups, and
    ...
    def clearImages(self) -> None:
        ...
        Side effects:
            Clears render caches and link groups, then renders the configured
            placeholder or blanks the viewer.
        """
        self._tile_manager.clear_caches()
        resources_cleared = getattr(self._qpane, "_catalog_resources_cleared", None)
        if callable(resources_cleared):
            resources_cleared()
    ...
    def enter_placeholder_mode(self) -> None:
        """Notify the host that placeholder interaction policy became active."""
        policy = self._placeholder_policy
        if policy is None:
            return
        self._placeholder_active = True
        entered = getattr(self._qpane, "_catalog_placeholder_entered", None)
        if callable(entered):
            entered(bool(policy.panzoom_enabled))
    ...
    def exit_placeholder_mode(self) -> None:
        ...
        if not self._placeholder_active:
            return
        self._placeholder_active = False
        exited = getattr(self._qpane, "_catalog_placeholder_exited", None)
        if callable(exited):
            exited()
    ...
    def _notify_resource_invalidated(
        self,
        image_ids: Iterable[uuid.UUID],
    ) -> None:
        """Notify the host that image-dependent domain products are stale."""
        invalidated = tuple(dict.fromkeys(image_ids))
        if not invalidated:
            return
        callback = getattr(self._qpane, "_catalog_image_resources_removed", None)
    ...
    def _remove_images(
        ...
        will_remove = getattr(self._qpane, "_catalog_image_will_remove", None)
        for image_id in ordered_ids:
            ...
            if callable(will_remove):
                will_remove(image_id)
        ...
        if not self.catalog.hasImages():
            ...
            removed_resources = getattr(
                self._qpane,
                "_catalog_image_resources_removed",
                None,
            )
            if callable(removed_resources):
                removed_resources(tuple(ids_to_evict))
            self._display_current_catalog_image()
