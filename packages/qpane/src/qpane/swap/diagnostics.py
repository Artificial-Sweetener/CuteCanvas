...
def _format_prefetch_line(metrics: SwapCoordinatorMetrics) -> str:
    ...
    parts: list[str] = []
    if metrics.pending_scene_prefetch > 0:
        parts.append(f"scene_sources={metrics.pending_scene_prefetch}")
    if metrics.pending_source_warmups > 0:
        parts.append(f"source_warmups={metrics.pending_source_warmups}")
