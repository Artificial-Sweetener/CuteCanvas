...
"""Track conservative panel damage from transient raster products."""
...
def transient_raster_transition_damage(
    previous: SceneRenderPlan | None,
    current: SceneRenderPlan,
) -> QRectF | None:
    """Return the union touched by previous and current transient products."""
    rectangles = tuple(
        rect
        for plan in (previous, current)
        if plan is not None
        if (rect := _transient_product_bounds(plan)) is not None
    )
    if not rectangles:
        return None
...
def _transient_product_bounds(plan: SceneRenderPlan) -> QRectF | None:
    """Project one plan's transient raster contribution into panel space."""
