...
_DOMAIN_PRIORITIES = {
    ...
    ("executor", _CORE_TIER): 30,
    ("executor", _DETAIL_TIER): 35,
    ("retry", _CORE_TIER): 40,
    ("retry", _DETAIL_TIER): 45,
}
...
class Diagnostics(QObject):
    ...
    view in control of render and viewport state while cache, swap, and
    ...
    def register_core_providers(
        self,
        stack_accessor: Callable[[], View | object | None],
        *,
        pyramid_accessor: Callable[[], object | None] | None = None,
    ) -> None:
        """Install renderer, viewport, and pyramid diagnostics providers.

        Args:
            stack_accessor: Callable returning either the active rendering
                presenter, its owning ``View``, or ``None`` during initialization.
            pyramid_accessor: Optional direct accessor for the source pyramid
                manager used by standalone viewers without a catalog facade.
        """
        self._stack_accessor = stack_accessor
        self._pyramid_accessor = pyramid_accessor
    ...
    def register_domain_provider(
        self,
        domain: str,
        provider: DiagnosticsProvider,
        *,
        tier: str = _DETAIL_TIER,
    ) -> None:
        """Attach one extension diagnostics provider to a named domain."""
        normalized = str(domain).strip().lower()
        if not normalized:
            raise ValueError("diagnostics domain must be non-empty")
        self._register_once(provider, domain=normalized, tier=tier)
    ...
    def _build_core_provider(self) -> DiagnosticsProvider:
        ...
        def _provider(_qpane: QPane) -> tuple[DiagnosticRecord, ...]:
            ...
            stack = accessor()
            presenter = (
                getattr(stack, "presenter", stack) if stack is not None else None
            )
            ...
            try:
                ...
                catalog = None
            pyramid_manager = (
                self._pyramid_accessor() if self._pyramid_accessor is not None else None
            )
            if pyramid_manager is None and catalog is not None:
