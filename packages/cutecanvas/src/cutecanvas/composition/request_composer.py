...
"""Validate catalog-backed composition requests and create document values."""
...
class CompositionRequestComposer:
    """Project public composition request values into the document model."""
    ...
    def __init__(
        ...
        *,
        ...
        """Bind catalog capability lookups used by composition requests."""
    ...
    def request_from_template(
        ...
        """Validate a template and expand it into one composition request."""
        if not isinstance(template, CompositionTemplate):
            raise TypeError("template must be a CompositionTemplate")
        if not isinstance(bindings, TemplateBindings):
            raise TypeError("bindings must be a TemplateBindings")
        ...
        layer_ids: set[uuid.UUID] = set()
        request_layers: list[CatalogLayerRequest] = []
    ...
    def _normalize_layer(
        self,
        layer: CatalogLayerRequest,
        *,
        catalog_contains: Callable[[uuid.UUID], bool],
    ) -> CompositionLayerInstance:
        """Validate one catalog layer and return its ordinary instance value."""
        if not isinstance(layer, CatalogLayerRequest):
            raise TypeError("scene layers must be CatalogLayerRequest instances")
        if not isinstance(layer.layer_id, uuid.UUID):
...
def _validate_clip(clip: CompositionLayerClip | None) -> None:
    """Validate clip geometry used by a public composition layer."""
    if clip is None:
        return
...
def _copy_scene_clip(clip: CompositionLayerClip | None) -> CompositionLayerClip | None:
    """Return a detached copy of a public scene clip."""
    if clip is None:
        return None
    return CompositionLayerClip(
        coordinate_space=clip.coordinate_space,
        rect=QRectF(clip.rect),
    )


def _internal_clip(clip: CompositionLayerClip | None) -> LayerClip | None:
