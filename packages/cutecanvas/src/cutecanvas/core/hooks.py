...
"""Hook interfaces for CuteCanvas feature installers.

Centralize the integration points exposed by the CuteCanvas facade so optional feature
...
if TYPE_CHECKING:  # pragma: no cover - import cycle guard
    from qpane.sdk.diagnostics import DiagnosticsProvider

    from ..canvas import CuteCanvas
    from ..masks.autosave import AutosaveManager


class ToolFactory(Protocol):
    """Factory callable used to create tool instances on demand."""

    def __call__(self) -> ViewerTool:
...
class ToolSignalBinder(Protocol):
    ...
    def __call__(self, signals: ToolManagerSignals, tool: ViewerTool) -> None:
        """Bind or unbind tool signals to the shared manager."""
        ...


class CursorProvider(Protocol):
    """Callable that returns a cursor for the current CuteCanvas state."""

    def __call__(self, qpane: CuteCanvas) -> QCursor | None:
        """Return the cursor to display for the active control mode."""
        ...


@dataclass
class CuteCanvasHooks:
    ...
    def registerCursorProvider(self, mode: str, provider: CursorProvider) -> None:
        ...
        Args:
            ...
            provider: Callable that inspects the CuteCanvas and returns a ``QCursor`` or
    ...
    def register_diagnostics_provider(
        ...
        *,
        ...
        """Register an additional diagnostics provider for the CuteCanvas overlay.

        Args:
            provider: Callable that yields :class:`~qpane.types.DiagnosticRecord`
                entries for the owning CuteCanvas.
