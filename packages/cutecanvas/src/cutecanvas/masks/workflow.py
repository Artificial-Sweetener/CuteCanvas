...
class _SamWorkflow:
    ...
    def attach_manager(self, sam_manager) -> None:
        """Attach the SAM manager to the CuteCanvas and delegate."""
...
class Masks:
    """Controller that owns mask and SAM orchestration on behalf of CuteCanvas."""
    ...
    def __init__(
        ...
        *,
        ...
        """Wire CuteCanvas collaborators plus mask and SAM delegates.
    ...
    def register_diagnostics(self, broker: Diagnostics) -> None:
        ...
        if self._diagnostics_registered:
            return
        broker.register_domain_provider(
            "mask", self._mask_summary_diagnostics, tier="core"
        )
        broker.register_domain_provider("mask", self._mask_job_diagnostics)
        broker.register_domain_provider("mask", self._mask_service_diagnostics)
        broker.register_domain_provider("mask", self._mask_brush_diagnostics)
        self._diagnostics_registered = True

    def _mask_summary_diagnostics(
    ...
    def _mask_job_diagnostics(self, qpane: CuteCanvas) -> tuple[DiagnosticRecord, ...]:
        """Return detailed mask job diagnostics rows."""
        return tuple(mask_job_detail_provider(qpane))

    def _mask_service_diagnostics(
        self, qpane: CuteCanvas
    ) -> tuple[DiagnosticRecord, ...]:
    ...
    def on_mask_saved(self, mask_id: str, path: str) -> None:
        """Relay autosave completions through the CuteCanvas signal surface."""
    ...
    def mask_delegate(self) -> MaskDelegate | None:
        """Return the mask delegate coordinating CuteCanvas interactions."""
    ...
    def sam_feature_available(self) -> bool:
        """Return True when SAM manager support is active for the host CuteCanvas."""
    ...
    def attachMaskService(self, service: MaskService) -> None:
        """Attach the mask service via MaskDelegate to avoid CuteCanvas indirection."""
    ...
    def detachMaskService(self) -> None:
        """Detach the mask service via MaskDelegate to avoid CuteCanvas indirection."""
        delegate = self._ensure_mask_delegate()
        service = self._ensure_mask_service()
