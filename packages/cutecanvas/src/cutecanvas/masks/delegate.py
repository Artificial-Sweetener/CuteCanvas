...
"""Controllers that keep mask-specific logic out of the CuteCanvas QWidget."""
...
class MaskDelegate:
    """Bridge CuteCanvas mask calls to MaskService, undo helpers, and autosave wiring."""
    ...
    def attachMaskService(self, service: MaskService) -> None:
        ...
        if registry is not None and controller is not None:
            registry.attach_extension(
                "mask_overlays",
                lambda coordinator: EvictableCacheConsumer(
                    controller.renders,
                    coordinator,
    ...
    # Public CuteCanvas API forwards
    ...
    def _mask_service(self) -> MaskService | None:
        """Expose the CuteCanvas-owned MaskService if present."""
