...
"""Transient content emphasis owned by the demo composition browser."""
...
class LayerBrowserHighlights:
    ...
    def _open_layer(self, target: LayerTarget | None) -> LayerHandle | None:
        """Resolve one active-composition layer handle without opening compositions."""
        if target is None:
            return None
        composition_id, layer_id = target
        composition = self._canvas.editor.compositions.get(composition_id)
        if composition is None or not composition.is_open:
            return None
        return composition.layer(layer_id)
