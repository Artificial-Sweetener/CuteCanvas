...
def test_transparent_storage_reservation_never_changes_source_bounds() -> None:
    """Allocation geometry must stay private until coverage becomes visible."""
    asset = CoverageAsset(
        uuid.uuid4(),
        CoverageSurface(
    ...
    assert asset.raster.set_bounds(RasterBounds(-500, -200, 1000, 400))
    assert asset.raster.bounds == RasterBounds(-500, -200, 1000, 400)
    assert asset.source_bounds() == RasterBounds(0, 0, 64, 64)

    storage_pixel = asset.raster.storage_rect(RasterBounds(100, 80, 1, 1))
    assert storage_pixel is not None
    asset.raster.mutate_storage_region(
        storage_pixel,
        lambda pixels, _image: pixels.fill(255),
    )

    assert asset.raster.content_bounds() == RasterBounds(100, 80, 1, 1)
    assert asset.source_bounds() == RasterBounds(0, 0, 101, 81)
