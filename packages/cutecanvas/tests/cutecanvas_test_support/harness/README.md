...
Expose the package-owned test support for the current repository-root shell:

```powershell
$env:PYTHONPATH = "packages\cutecanvas\tests"
```

Run the bounded cross-device profile used by GitHub Actions:

```powershell
.venv\Scripts\python -m cutecanvas_test_support.harness --profile ci
```

Run a longer local campaign over 4096-pixel images and ten deterministic seeds:

```powershell
.venv\Scripts\python -m cutecanvas_test_support.harness --profile soak
```

Campaign bounds can be overridden while retaining deterministic generation:

```powershell
.venv\Scripts\python -m cutecanvas_test_support.harness --profile soak --seed 200 --seeds 25 --random-strokes 60 --image-size 4096
```
...
```powershell
.venv\Scripts\python -m cutecanvas_test_support.harness --replay qpane-abuse-artifacts\seed-200\trace.json --image-size 4096
```
...
```powershell
.venv\Scripts\python -m cutecanvas_test_support.harness --replay qpane-abuse-artifacts\seed-200\trace.json --image-size 4096 --minimize
```
