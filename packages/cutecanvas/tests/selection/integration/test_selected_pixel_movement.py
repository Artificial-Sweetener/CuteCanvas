...
def test_selected_pixel_transform_frame_uses_complete_selection_bounds() -> None:
    """Transparent selected regions must remain authoritative transform geometry."""
    ...
    before = movement.transform_box_state()

    assert before is not None
    assert before.bounds.x == 0.0
    assert before.bounds.y == 0.0
    assert before.bounds.width == 6.0
    assert before.bounds.height == 2.0
    assert movement.begin_transform(
        TransformOperation(TransformOperationKind.SCALE, TransformHandle.RIGHT),
        QPointF(6.0, 1.0),
    )
    lifted = movement.transform_box_state()
    assert lifted is not None
    assert lifted.bounds == before.bounds


def test_affine_selected_pixel_preview_transforms_complete_selection() -> None:
    """Affine preview ants must follow the selection frame rather than tight pixels."""
    ...
    assert movement.begin_transform(operation, QPointF(6.0, 1.0))
    assert movement.update_transform(
        QPointF(12.0, 1.0),
        TransformModifiers(proportional=False),
    )
    ...
    assert preview.coverage.bounds == RasterBounds(0, 0, 12, 2)
...
class _FloatingAwareMovementResolver:
    """Resolve selected pixels before lift and floating pixels afterward."""

    def __init__(self, pixels: SelectedPixelMovementController) -> None:
        """Retain the authoritative floating-session owner."""

        self._pixels = pixels

    def resolve(
        self,
        _operation: EditorOperation,
        **_kwargs: object,
    ) -> EditorOperationResolution:
        """Return the movement consequence that pressing would have now."""
        ...
        return EditorOperationResolution(
            ...
            target=(
                ...
                if self._pixels.active
                else EditorOperationTarget.SELECTED_PIXELS
            ),
        )
...
def test_move_cursor_stops_advertising_cut_once_pixels_are_floating() -> None:
    """Scissors must describe the next press consequence, not selected-pixel hover."""

    pixels, _edits, _assets, _asset, _selection, _scene, _layer, _previews = (
        _movement_fixture()
    )
    interaction = EditorMovementInteraction(
        pixels=pixels,
        layers=_LayerMovementSpy(),
        operations=_FloatingAwareMovementResolver(pixels),
        panel_to_scene=lambda point: point,
        refresh_preview=lambda: None,
        snapping=_MovementSnapSpy(),
        configuration=MoveToolConfiguration(),
    )
    origin = QPointF(1.5, 0.5)

    assert interaction.update_hover(origin)
    assert interaction.cursor_intent is EditorCursorIntent.MOVE_CUT
    assert interaction.begin(origin)
    assert interaction.cursor_intent is EditorCursorIntent.MOVE
    assert interaction.finish(QPointF(2.5, 0.5))
    interaction.update_hover(QPointF(2.5, 0.5))
    assert interaction.cursor_intent is EditorCursorIntent.MOVE
