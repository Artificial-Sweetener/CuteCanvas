...
def test_first_shape_gesture_uses_pointer_alt_for_square_replacement() -> None:
    """Alt should constrain the first selection even without a prior key event."""
...
def test_delete_shortcut_requires_selection_and_successful_pixel_clear() -> None:
    """Delete should be claimed only by a clearable authoritative selection."""

    selected = {"value": False}
    clears: list[None] = []
    shortcuts = EditorSelectionShortcuts(
        has_selection=lambda: selected["value"],
        clear_selected_pixels=lambda: clears.append(None) or True,
    )
    without_selection = QKeyEvent(QEvent.Type.KeyPress, Qt.Key_Delete, Qt.NoModifier)
    without_selection.ignore()

    assert not shortcuts.handle(without_selection)
    assert not without_selection.isAccepted()
    assert not clears

    selected["value"] = True
    with_selection = QKeyEvent(QEvent.Type.KeyPress, Qt.Key_Delete, Qt.NoModifier)

    assert shortcuts.handle(with_selection)
    assert with_selection.isAccepted()
    assert clears == [None]


def test_alt_subtracts_without_constraining_when_selection_exists() -> None:
    """Alt should return to subtraction after the first selection exists."""
    ...
    assert commits[0].geometry.local_bounds == (20.0, 20.0, 10.0, 5.0)


def test_first_selection_alt_constraint_tracks_live_pointer_modifiers() -> None:
    """Pressing Alt after the drag begins should still produce a 1:1 shape."""
    ...
    assert len(commits) == 1
    assert commits[0].geometry.local_bounds == (20.0, 20.0, 10.0, 10.0)


def test_alt_keeps_first_lasso_in_replacement_mode() -> None:
    """Alt should not request subtraction when a first lasso has no selection."""
    ...
    modifiers = Qt.KeyboardModifier.AltModifier
    points = [QPointF(2.0, 2.0), QPointF(8.0, 2.0), QPointF(5.0, 9.0)]
    tool.mousePressEvent(
        _event(
            QEvent.Type.MouseButtonPress,
            points[0],
...
def test_first_selection_alt_cursor_has_no_subtraction_marker() -> None:
    """Alt should advertise geometric constraint before subtraction is possible."""
