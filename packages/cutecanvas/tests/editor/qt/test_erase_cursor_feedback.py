...
def test_cursor_builder_normalizes_transient_invalid_display_scale(qapp) -> None:
    """Cursor raster allocation must remain paintable while Qt changes screens."""

    cursor = CursorBuilder().create_precision_cursor(device_pixel_ratio=0.0)

    assert not cursor.pixmap().isNull()
    assert cursor.pixmap().size() == QSize(40, 40)
