...
class _TransientScaleCursorCanvas(_CursorCanvas):
    """Simulate Qt briefly reporting an unusable display scale."""

    def devicePixelRatioF(self) -> float:
        """Return the transient invalid value seen during window transitions."""
...
def test_host_theme_never_receives_transient_invalid_display_scale(qapp) -> None:
    """Cursor arbitration must normalize DPR before invoking host rendering."""

    canvas = _TransientScaleCursorCanvas()
    controller = EditorCursorController(
        cast("CuteCanvas", canvas),
        cursor_suppressed=lambda: False,
    )
    ...
    assert theme.requests == [(EditorCursorIntent.SELECTION_TRANSLATE, 1.0)]
