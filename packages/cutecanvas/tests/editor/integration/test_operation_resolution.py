...
from cutecanvas.editor.operation_contracts import (
    EditorOperation,
    EditorOperationAlternative,
    EditorOperationDenial,
    EditorOperationTarget,
)
from cutecanvas.editor.operation_resolution import EditorOperationResolver
from cutecanvas.editor.source_operations import (
...
class _EmptyContentBoundsOwner:
    """Represent a source domain that authoritatively reports no content."""

    @staticmethod
    def content_bounds(_source):
        """Return no bounds because the source contains no meaningful pixels."""
        return
...
def _resolver_fixture(
    *,
    ...
    source_capabilities = EditorSourceCapabilities.create()
    if empty_content:
        source_capabilities.content_bounds.register(
            ProjectResourceReference,
            _EmptyContentBoundsOwner(),
        )
    layer_geometry = LayerGeometryResolver(
        source_capabilities,
        lambda _layer: LayerGeometryPolicy(),
    )
...
def test_empty_capable_source_denies_layer_transform_without_fallback_bounds() -> None:
    """Supported empty content must not inherit the descriptor storage envelope."""
    resolver, scene_id, layer_id = _resolver_fixture(
        placed=False,
        empty_content=True,
    )

    resolution = resolver.resolve_transform_target(EditorOperationTarget.LAYER)

    assert not resolution.allowed
    assert resolution.scene_id == scene_id
    assert resolution.layer_id == layer_id
    assert resolution.denial is EditorOperationDenial.NOTHING_TO_TRANSFORM
