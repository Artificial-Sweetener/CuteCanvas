...
def test_color_renderer_respects_target_local_affine_tip_geometry() -> None:
    """Color rendering must reserve and paint the same transformed tip shape."""
    bounds = QRect(0, 0, 128, 128)
    segment = BrushStrokeSegment(
        start=(64.0, 64.0),
        end=(64.0, 64.0),
        start_diameter=40.0,
        end_diameter=40.0,
        hardness=1.0,
        tip_transform=LayerTransform(m11=0.5, m22=2.0),
    )

    painted = render_color_stroke(
        before=np.zeros((128, 128, 4), dtype=np.uint8),
        patch_bounds=bounds,
        segments=(segment,),
        color=QColor(220, 80, 40, 255),
    )
    ys, xs = np.nonzero(painted[:, :, 3])

    assert 18 <= int(xs.max() - xs.min() + 1) <= 22
    assert 78 <= int(ys.max() - ys.min() + 1) <= 82
...
def test_clone_stamp_source_matches_marker_with_nonzero_raster_origin(
    ...
    assert painting.begin()
    assert painting.apply(
        BrushStrokeSegment.fixed((40.0, 16.0), (40.0, 16.0), 1.0, False)
    )
    assert painting.commit()
