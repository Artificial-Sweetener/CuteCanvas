...
def test_delete_key_clears_selection_independently_of_active_tool(
    ...
    """Selection state should own Delete regardless of the persistent tool."""
    qpane, _manager, _image_id = qpane_with_mask
    scene = qpane.currentScene()
    assert scene is not None
    layer_id = qpane.addEditableRasterLayer(
    ...
    QTest.keyClick(qpane, Qt.Key_Delete)
    cleared_from_brush = qpane.editableRasterLayerImage(scene.scene_id, layer_id)
    assert cleared_from_brush is not None
    assert cleared_from_brush.pixelColor(1, 1).alpha() == 0
    assert cleared_from_brush.pixelColor(6, 1).alpha() == 255
