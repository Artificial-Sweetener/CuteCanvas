...
def test_demo_delete_shortcut_clears_selected_pixels_from_moved_mask(
    ...
    try:
        ...
        selected_bounds = RasterBounds(20, 180, 180, 40)
        preserved_bounds = RasterBounds(201, 0, 199, size)
        preserved_before = layer.coverage.raster.capture_region(preserved_bounds)
        ...
        assert not np.any(layer.coverage.raster.capture_region(selected_bounds))
        assert np.array_equal(
            layer.coverage.raster.capture_region(preserved_bounds),
            preserved_before,
        )
