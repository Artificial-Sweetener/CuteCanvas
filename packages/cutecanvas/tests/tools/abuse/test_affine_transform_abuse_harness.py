...
def test_explicit_transform_targets_share_commands_without_sharing_frames(
    qapp: QApplication,
) -> None:
    """Selection and layer routes must retain distinct bounds in one session API."""
    harness = MountedQPaneHarness(
        qapp,
    ...
    viewer = harness.viewer
    try:
        raster = QImage(96, 96, QImage.Format_ARGB32_Premultiplied)
        raster.fill(Qt.transparent)
        painter = QPainter(raster)
        painter.fillRect(QRect(28, 30, 12, 8), QColor(220, 40, 90, 255))
        painter.end()
        layer_id = viewer.addEditableRasterLayer(
            raster,
            interaction=LayerPolicy(
        ...
        assert scene is not None and layer_id is not None
        assert viewer.setSelectedLayer(scene.scene_id, layer_id)
        selection = QImage(44, 24, QImage.Format_Grayscale8)
        selection.fill(255)
        assert viewer.setPixelSelection(selection, QRect(10, 14, 44, 24))

        selection_state = viewer.editorTransformState(
            EditorTransformTarget.SELECTION_CONTENT
        )
        assert selection_state.allowed and selection_state.corners is not None
        assert selection_state.corners[0] == QPointF(10.0, 14.0)
        assert selection_state.corners[2] == QPointF(54.0, 38.0)
        ...
        drag_origin = presentation.center.toPoint()
        QTest.mousePress(viewer, Qt.LeftButton, Qt.NoModifier, drag_origin)
        harness.drain_events()
        dragging = viewer.editorTransformState(EditorTransformTarget.SELECTION_CONTENT)
        assert dragging.gesture_active
        QTest.mouseRelease(viewer, Qt.LeftButton, Qt.NoModifier, drag_origin)
        harness.drain_events()
        settled = viewer.editorTransformState(EditorTransformTarget.SELECTION_CONTENT)
        assert not settled.gesture_active
        assert viewer.applyEditorTransformCommand(
            EditorTransformCommand.ROTATE_RIGHT_90
        )
        rotated = viewer.editorTransformState(EditorTransformTarget.SELECTION_CONTENT)
        assert rotated.unresolved and rotated.center == selection_state.center
        assert viewer.cancelEditorTransform()
        assert viewer.editableRasterLayerImage(scene.scene_id, layer_id) == raster

        assert viewer.clearPixelSelection()
        layer_state = viewer.editorTransformState(EditorTransformTarget.LAYER_CONTENT)
        assert layer_state.allowed and layer_state.corners is not None
        assert layer_state.corners[0] == QPointF(28.0, 30.0)
        assert layer_state.corners[2] == QPointF(40.0, 38.0)
        original_transform = viewer.layerTransform(scene.scene_id, layer_id)
        assert viewer.activateEditorTransform(EditorTransformTarget.LAYER_CONTENT)
        assert viewer.applyEditorTransformCommand(
            EditorTransformCommand.FLIP_HORIZONTAL
        )
        assert viewer.layerTransform(scene.scene_id, layer_id) == original_transform
        assert viewer.applyEditorTransform()
        flipped_transform = viewer.layerTransform(scene.scene_id, layer_id)
        assert flipped_transform is not None and flipped_transform != original_transform
        assert viewer.undoSceneEdit()
        assert viewer.layerTransform(scene.scene_id, layer_id) == original_transform
    finally:
        harness.close()
...
def test_empty_legacy_mask_extent_cannot_create_a_layer_transform_frame(
    qapp: QApplication,
) -> None:
    """An empty restored mask must ignore a stale oversized storage envelope."""

    harness = MountedQPaneHarness(
        qapp,
        image_size=QSize(2160, 3072),
        widget_size=QSize(720, 720),
        mask_count=1,
        cache_budget_mb=96,
    )
    viewer = harness.viewer
    ...
    try:
        ...
        mask = viewer.mask_service.assets.get_layer(mask_id)
        assert mask is not None
        assert mask.coverage.content_bounds() is None
        assert mask.coverage.raster.set_bounds(RasterBounds(-6480, -67584, 8640, 98304))
        ...
        state = viewer.editorTransformState(EditorTransformTarget.LAYER_CONTENT)

        assert not state.allowed
        assert state.denial == "nothing-to-transform"
        assert not viewer.activateEditorTransform(EditorTransformTarget.LAYER_CONTENT)
        assert viewer.sceneLayerTransformInteraction().presentation() is None
    finally:
        harness.close()


def test_idle_selected_pixel_transform_emits_no_qt_paint_diagnostics(
    qapp: QApplication,
) -> None:
    """An idle animated selection must never reenter a paint device."""
