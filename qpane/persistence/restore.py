...
class CompositionArchiveRestorer:
    ...
    def restore(self, archive: CompositionArchiveSnapshot) -> None:
        ...
        previous_masks: dict[uuid.UUID, CoverageSnapshot | None] = {
            mask_id: self._snapshot(mask_id) for mask_id in mask_ids
        }
        raster_ids = self._raster_ids(archive)
        previous_rasters: dict[uuid.UUID, ColorRasterSnapshot | None] = {
    ...
    def _snapshot(self, mask_id: uuid.UUID) -> CoverageSnapshot | None:
    ...
    def _raster_snapshot(self, raster_id: uuid.UUID) -> ColorRasterSnapshot | None:
        """Return one pre-restore color surface snapshot when it exists."""
        asset = self._rasters.get(raster_id)
        return None if asset is None else asset.surface.snapshot()
