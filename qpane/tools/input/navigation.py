...
class TouchNavigationSession:
    ...
    def update(
        ...
        *,
        ...
        """Apply the latest contact geometry or reset when no contacts remain."""
        ...
        dpr = self._safe_dpr()
        baseline_centroid_physical = QPointF(
            baseline_centroid.x() * dpr,
            baseline_centroid.y() * dpr,
        )
        current_centroid_physical = QPointF(
            current_centroid.x() * dpr,
            current_centroid.y() * dpr,
        )
        panel_center = self._qpane.physicalViewportRect().center()
        content_offset = baseline_centroid_physical - panel_center - self._baseline_pan
        if self._baseline_zoom > 0:
            content_offset /= self._baseline_zoom
        pan = current_centroid_physical - panel_center - content_offset * requested_zoom
    ...
    def finish(self) -> bool:
        ...
        viewport = self._qpane.view().viewport
        enabled = bool(getattr(self._qpane.settings, "touch_inertia_enabled", True))
        deceleration = float(
            getattr(self._qpane.settings, "touch_inertia_deceleration", 4500.0)
        )
        self.reset()
    ...
    def _rebaseline(
        ...
        *,
        ...
        """Anchor subsequent deltas to the current committed viewport state."""
        viewport = self._qpane.view().viewport
    ...
    def _safe_dpr(self) -> float:
        """Return a positive device pixel ratio for logical-to-physical deltas."""
        dpr = float(self._qpane.devicePixelRatioF())
        return dpr if math.isfinite(dpr) and dpr > 0 else 1.0

    @staticmethod
    def _centroid(points: Mapping[int, QPointF]) -> QPointF:
        """Return the arithmetic center of the supplied contacts."""
        count = len(points)
        return QPointF(
    ...
    def _record_translation_velocity(
        ...
        """Record one physical translation velocity sample for kinetic release."""
    ...
    def _average_translation_velocity(self) -> QPointF:
        """Return a recency-weighted velocity across recent touch frames."""
    ...
    def _timestamp_seconds(timestamp_ms: int | None) -> float:
        """Return a monotonic timestamp, preferring meaningful Qt event time."""
