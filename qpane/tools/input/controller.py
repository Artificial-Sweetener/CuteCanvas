...
class PointerInputController(QObject):
    ...
    def __init__(
        ...
        *,
        ...
        self._navigation = TouchNavigationSession(qpane)
        ...
        if self._touch_divider_claimed:
            if primary_sample is not None:
                ...
                if primary_sample.phase is PointerPhase.END:
                    divider.handle_touch_end(primary_sample.position)
                else:
                    divider.handle_touch_update(primary_sample.position)
    ...
    def handle_tablet_event(self, event: QTabletEvent) -> bool:
        ...
        tool = qpane._tools_manager.get_active_tool()
    ...
    def pen_suppresses_touch_tool(self) -> bool:
        ...
        if self._pen_last_seen_at is None:
            return False
        rejection_window = max(0, int(self._qpane.settings.palm_rejection_ms)) / 1000.0
    ...
    def _touch_navigation_allowed(self) -> bool:
        """Return whether the current QPane state permits one-finger navigation."""
        qpane = self._qpane
        if qpane._is_blank or not qpane.view().has_renderable_content():
            return False
        if qpane.view().viewport.is_locked():
            return False
        return bool(qpane.settings.touch_navigation_enabled)
