...
from ..coverage import CoverageCombineMode, CoverageSnapshot, combine_coverage
from ..scene.model import LayerDescriptor, SceneDescriptor
from ..scene.pixel_extraction import build_pixel_lift
from ..scene.pixel_fragments import (
    RasterPixelFormat,
    RasterPixelFragment,
    RasterPixelLift,
)
...
class MaskPixelRenderSynchronizer:
    ...
    def refresh(self, mask_id: uuid.UUID, local_bounds: RasterBounds) -> None:
        ...
        if layer is None:
            return
        storage_bounds = layer.surface.storage_rect(local_bounds)
...
class MaskLayerPixelMutationOwner:
    ...
    def extent_policy(self, layer: LayerDescriptor) -> RasterExtentPolicy | None:
        ...
        return None if mask is None else mask.surface.extent_policy
    ...
    def capture_patch(
        ...
        if (
            ...
            return None
        return mask.surface.snapshot_storage_region(storage)
    ...
    def clear_coverage(
        ...
        if storage is None or coverage.pixels.shape != (storage.height, storage.width):
            return False
        current = mask.surface.snapshot_storage_region(storage)
...
def _capture_mask_region(mask: MaskLayer, bounds: RasterBounds) -> np.ndarray:
    ...
    surface_bounds = mask.surface.bounds
    ...
    if storage is None:
        return pixels
    source = mask.surface.snapshot_storage_region(storage)
