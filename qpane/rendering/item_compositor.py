...
class SceneItemCompositor:
    ...
    def draw_visible_items(self, painter: QPainter, plan: SceneRenderPlan) -> None:
        """Draw visible scene items in bottom-to-top order."""
        index = 0
        while index < len(plan.render_items):
            item = plan.render_items[index]
            ...
            if isinstance(item, RasterLayerRenderItem) and preview is not None:
                group_end = index + 1
                while (
                    group_end < len(plan.render_items)
                    and isinstance(plan.render_items[group_end], RasterLayerRenderItem)
                    and plan.render_items[group_end].descriptor.layer_id
                    == item.descriptor.layer_id
                ):
                    group_end += 1
                group = tuple(
                    candidate
                    for candidate in plan.render_items[index:group_end]
                    if isinstance(candidate, RasterLayerRenderItem)
                    and candidate.descriptor.visible
                )
                painter.save()
                try:
                    self._draw_floating_layer_group(painter, plan, group, preview)
                finally:
                    painter.restore()
                index = group_end
                continue
            painter.save()
            try:
                self.draw_item(painter, plan, item)
            finally:
                painter.restore()
            index += 1
    ...
    def draw_item(
        ...
        if isinstance(item, RasterLayerRenderItem):
            self.draw_raster_item(painter, plan, item)
    ...
    def draw_raster_item(
        ...
        if isinstance(preview, FloatingPixelResolvedContribution):
            ...
            return
        if isinstance(preview, FloatingPixelTransformContribution):
            self._draw_isolated_floating_item(
                painter,
                plan,
                item,
                preview,
        ...
        self._apply_layer_clip(painter, plan, item)
        self._apply_layer_effects(painter, item)
        self.draw_raster_source(painter, plan, item)
    ...
    def _draw_isolated_floating_item(
        self,
        painter: QPainter,
        plan: SceneRenderPlan,
        item: RasterLayerRenderItem,
        preview: FloatingPixelTransformContribution,
        ...
        try:
            ...
            layer_painter.setTransform(item.transform, True)
            ...
            self._apply_floating_products(layer_painter, item, preview)
        finally:
            layer_painter.end()
        painter.save()
        try:
    ...
    def _draw_floating_layer_group(
        self,
        painter: QPainter,
        plan: SceneRenderPlan,
        items: tuple[RasterLayerRenderItem, ...],
        preview: FloatingPixelRenderContribution,
    ) -> None:
        ...
        try:
            ...
            try:
                ...
                layer_painter.setTransform(reference.transform, True)
