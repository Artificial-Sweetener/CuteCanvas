...
*   **Composition (`qpane/composition/`)**: `CompositionService` owns public composition records, browser order, generated default image compositions, layered scene composition records, active composition selection, and composition-scoped comparison state. `ImageSceneLayerStore` owns catalog-backed scene layer instances, cross-kind z-order, visibility, presentation, hit-test metadata, interaction policy, placement, and labels. Composition mutation owners apply authoritative layer changes, while `LayerPlacementHistory` owns scene-scoped placement undo and redo.
*   **Scene (`qpane/scene/`)**: Internal scene descriptors, layer descriptors, layer sources, placement, interaction and hit-test metadata, scene/layer identity, provider/resolver contracts, and internal scene mutation routing. `SceneLayerSelectionController` owns stable generic layer selection, `SceneLayerPlacementPreview` owns transient render-only placement, `SceneLayerMovementController` coordinates policy-checked selection, preview, and placement commits without knowing source domains, and `SceneLayerMovementInteraction` adapts panel coordinates and publication callbacks.
*   **Rendering (`qpane/rendering/`)**: `SceneAssembly` owns complete ordered scene construction. `RenderingPresenter` consumes assembled scenes for draw orchestration and render-work planning. Layer-source resolvers supply pixels and derived rasters without presenter-to-feature reach-through. `Viewport` owns persistent pan/zoom state, transforms, coordinate conversion, and authoritative constraints. `ViewportMotionController` owns transient kinetic translation and its timer lifecycle. Visibility planning owns layer-visible source geometry used to cull tile work before painting.
*   **Tools (`qpane/tools/`)**: `ToolInteractionDelegate` owns widget plumbing, tool activation, and OS-cursor arbitration. The pointer-input subsystem owns Qt mouse/touch/tablet normalization, physical-modality transitions, application-level pen proximity, sequence capture, synthesized-mouse rejection, gesture arbitration, palm rejection, touch navigation geometry, and routing from declarative `ToolInputProfile` capabilities. Tool classes own tool-specific gesture translation; `MoveTool` delegates generic layer movement without owning scene state. `BrushStrokeSession` owns captured brush-contact state and segment formation without owning mask persistence. `BrushTool` owns semantic brush-feedback state, while `BrushPreviewRenderer` owns its canvas rendering without owning input policy or mask persistence.
*   **Masks (`qpane/masks/`)**: `MaskAssetStore` owns mask IDs and authoritative `MaskSurface` pixels; surfaces expose detached snapshots and controlled mutations with one content revision. `MaskHistory` owns undo providers, commands, limits, and replay. `MaskEditService` owns transactional pixel edits and the sole async edit epochs. `MaskRenderCache` and `MaskRasterizer` own derived overlay rasters, cache policy, and render metrics, while `MaskRenderWorkCoordinator` owns asynchronous render scheduling, cancellation, and completion. `MaskLayerCoordinator` adapts composition-owned layer instances and scene mutation routing; `MaskLayerWorkflow` coordinates import, creation, removal, and stack commands without owning pixels or presentation state. `MaskActivationController` owns editable-layer selection and deferred activation, `ActiveMaskLayerCoordinates` maps editing through the resolved mask transform, and `MaskAutosaveCoordinator` owns autosave installation and signals. `MaskService` is the host-facing feature facade that wires and delegates to these owners. `MaskCombiner` and `MaskComponentAdjustmentTool` own focused reusable mask operations; the factory SAM tool consumes component adjustment. Stroke payload models own immutable UI-to-worker contracts, while `MaskStrokePipeline` owns previews, dirty regions, worker submission, and stroke commit ordering. `MaskLayerSourceResolver` supplies mask rasters through the generic scene resolver registry.
