...
*   **Masks (`qpane/masks/`)**: `MaskService` owns mask workflows, state transitions, async mask operations, autosave, and undo integration. Stroke payload models own immutable UI-to-worker data contracts, while `MaskStrokePipeline` owns previews, dirty regions, worker submission, and stroke commit ordering.
