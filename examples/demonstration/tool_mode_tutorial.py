...
The controller groups mode actions, destination validation, placeholder
policy, and contextual tool controls. CuteCanvas remains authoritative for
the active mode and all editable state.
"""
...
from examples.demonstration.coverage_controls import CoverageControls
...
class ToolModeTutorialController:
    ...
    @staticmethod
    def placeholder_panzoom_enabled(settings: Mapping[str, object]) -> bool:
        """Return whether a host permits navigation over its placeholder."""
        placeholder = settings.get("placeholder")
        return isinstance(placeholder, Mapping) and bool(
            placeholder.get("panzoom_enabled", False)
        )
    ...
    def set_mode(self, mode: str) -> None:
        ...
        if mode == CuteCanvas.CONTROL_MODE_DRAW_BRUSH:
            ...
            if selected_layer is not None and selected_layer.source_kind in {
                ...
                target = self._canvas.paintTargetState()
            if target is None and self._masks_available():
        ...
        self.sync_mode(mode)
        self._show_status(f"Mode: {self.describe_mode(mode)}")
        self._catalog_refresh()

    def cycle_mode(self) -> None:
        """Advance through modes that are valid for the current host state."""
        placeholder_active = self._canvas.placeholderActive()
        panzoom_allowed = (not placeholder_active) or self.placeholder_panzoom_enabled(
            self._canvas.settings.as_dict()
        )
        preferred = [
    ...
    def catalog_prefers_mask_selection(self) -> bool:
        """Return whether the browser should highlight the active mask."""
        return self._canvas.getControlMode() in {
            CuteCanvas.CONTROL_MODE_MOVE,
            CuteCanvas.CONTROL_MODE_TRANSFORM,
            CuteCanvas.CONTROL_MODE_DRAW_BRUSH,
    ...
    def apply_catalog_focus(self, kind: str) -> None:
        """Choose a conventional tool when the browser changes target kind."""
        current = self._canvas.getControlMode()
        if kind == "mask":
            if current not in {
                CuteCanvas.CONTROL_MODE_MOVE,
        ...
        elif kind == "image" and current not in {
            CuteCanvas.CONTROL_MODE_CURSOR,
            CuteCanvas.CONTROL_MODE_PANZOOM,
            CuteCanvas.CONTROL_MODE_MOVE,
            CuteCanvas.CONTROL_MODE_TRANSFORM,
    ...
    def sync_mode(self, mode: str) -> None:
        ...
        self.mode_transform_action.setChecked(mode == CuteCanvas.CONTROL_MODE_TRANSFORM)
    ...
    def refresh_availability(self) -> None:
        """Apply host feature and placeholder policy to all tool controls."""
        placeholder = self._canvas.placeholderActive()
        panzoom = (not placeholder) or self.placeholder_panzoom_enabled(
            self._canvas.settings.as_dict()
        )
        mask_available = self._masks_available()
        ...
        enable(self.mode_move_action, not placeholder)
        enable(self.mode_transform_action, not placeholder)
        enable(self.mode_brush_action, not placeholder)
        enable(self.mode_smart_action, sam_available and not placeholder)
        custom_action, lens_action = self._extension_actions()
        enable(custom_action, custom_action is not None and not placeholder)
        enable(lens_action, lens_action is not None and not placeholder)
        ...
        if sam_available and not placeholder:
            allowed.add(CuteCanvas.CONTROL_MODE_SMART_SELECT)
        if custom_action is not None and not placeholder:
            allowed.add(CUSTOM_TOOL_MODE)
        if lens_action is not None and not placeholder:
        ...
        self._tool_state = (placeholder, panzoom)
        if placeholder and previous != self._tool_state:
            disabled: list[str] = []
            if mask_available:
                disabled.append("mask tools")
            if sam_available and self.mode_smart_action is not None:
                disabled.append("smart select")
            if not panzoom:
                disabled.append("pan/zoom")
            message = (
                f"Placeholder showing: {', '.join(disabled)} are unavailable until you load an image."
                if disabled
                else "Placeholder showing. Pan/zoom follows your placeholder settings."
            )
            if not panzoom:
                message += " Enable pan/zoom in Config > Placeholder to inspect it."
            self._show_status(message)
        elif previous and previous[0] and not placeholder:
            self._show_status("Image selected; tools are available again.")
    ...
    def _create_mode_actions(self) -> None:
        ...
        self.mode_move_action.setStatusTip(
            "Drag selected pixels or, without a selection, a movable layer."
        )
        self.mode_transform_action = QAction("Transform", self._parent, checkable=True)
        self.mode_transform_action.setShortcut(QKeySequence("Ctrl+T"))
        self.mode_transform_action.setStatusTip(
            "Transform the selected layer; Enter applies and Escape cancels."
        )
        ...
        bindings = [
            ...
            (self.mode_transform_action, CuteCanvas.CONTROL_MODE_TRANSFORM),
    ...
    def _ensure_active_mask(self) -> bool:
        ...
        if image is None or image.isNull():
            self._show_status("Load an image before using mask tools.")
            return False
