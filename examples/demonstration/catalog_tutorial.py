...
"""Teach a document-and-layer tree owned by the host interface.
...
dialogs without copying document state.
"""
...
class CatalogTutorialController:
    ...
    def __init__(
        ...
        *,
        ...
        show_mask_selection: Callable[[], bool],
        focus_requested: Callable[[str], None],
        show_status: Callable[[str], None],
        set_delete_mask_enabled: Callable[[bool], None],
    ) -> None:
        ...
        self._set_delete_mask_enabled = set_delete_mask_enabled
        ...
        self.dock: CatalogDock | None = None
    ...
    def build(self) -> None:
        ...
        self.dock = CatalogDock(
            self._canvas,
            show_mask_selection=self._show_mask_selection,
    ...
    def show_initially(self) -> None:
        """Reveal the browser for the tutorial's starter document."""
    ...
    def handle_snapshot(self, snapshot: CatalogSnapshot) -> None:
        """React to derived mask presence without storing catalog state."""
        has_masks = any(
            image.masks for group in snapshot.groups for image in group.images
        )
        if has_masks and not self._contains_masks:
            self.maybe_auto_show(snapshot.image_count, force=True)
        self._set_delete_mask_enabled(has_masks)
        self._contains_masks = has_masks
