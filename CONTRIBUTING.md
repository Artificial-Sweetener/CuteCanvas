...
- **Composition (`qpane/composition/`)**: `CompositionService` owns composition records and active-composition state. `ImageSceneLayerStore` owns all layer instances and their cross-kind order and presentation. `CompositionEditHistory` is the sole chronological, byte-budgeted undo/redo store for editor commands, and `CompositionEditController` routes those commands to domain handlers and publishes scope-level history availability after chronology changes.
- **Scene (`qpane/scene/`)**: Internal scene and layer descriptors, typed layer sources, cross-kind assembly, derived placement, interaction and hit-test metadata, scene/layer identity, provider/resolver contracts, and mutation routing. `RasterBounds` owns integer source-local extents, `LayerTransform` maps local coordinates independently into scene space, `RasterLayerMutationCoordinator` routes generic raster structure requests, and `LayerPixelOwnerRegistry` is the sole source-to-pixel-owner mapping used for source-domain content occupancy and generic pixel mutation. Generic layer selection, transient placement preview, and movement coordination remain source-neutral.
- **Coverage (`qpane/coverage/`)**: `CoverageSurface` owns source-neutral 8-bit grayscale coverage pixels, integer local bounds, fixed/expand-on-write policy, synchronized region access, algebra, snapshots, and content/structure revisions.
...
- **Raster (`qpane/raster/`)**: `EditableRasterAssetStore` owns premultiplied RGBA assets. Descriptor and resolver owners adapt them into scenes, the pixel-mutation owner applies selection-constrained edits, and the structure-mutation owner exclusively coordinates asynchronous bounds changes and history commands.
- **Editor (`qpane/editor/`)**: `EditorInteractionCoordinator` coordinates source-neutral layer selection, pixel selection, source-coverage projection, selection-constrained deletion, and mask-stroke constraints by delegating to the authoritative scene, selection, and source-domain owners. `SelectedPixelMoveTargetResolver` owns movement eligibility. `FloatingPixelSession` owns unresolved pointer and cut/copy displacement state plus the exact transition for its current position, `SelectedPixelMovementController` coordinates input and transition composition, and `FloatingPixelResolutionOwner` applies explicit source, compatible-destination, or new-layer outcomes. `FloatingLayerPromotionRegistry` routes new-layer lifecycle to source domains, and `FloatingPixelHistory` replays every raster, selection, selected-layer, and layer-lifecycle transition atomically. `EditorMovementInteraction` arbitrates that branch ahead of whole-layer placement and distinguishes temporary input suspension from explicit cancellation. None duplicates durable source state.
- **Rendering (`qpane/rendering/`)**: `SceneAssembly` owns complete ordered scene construction. `RenderingPresenter` consumes assembled scenes for the draw pipeline and render-work planning. `FloatingPixelRenderCompiler` adapts exact source-neutral transitions through registered layer-source presentation owners, while `Renderer` owns the handoff to matching durable raster revisions. `Viewport` owns pan/zoom state and transforms. Visibility planning owns layer-visible source geometry used to cull tile work before painting.
