...
//! Responsibility: Define the typed spatial contract for immutable raster source revisions.
//!
//! Does not own: source storage, revision lookup, graph source bindings, scheduling, or copying.

use std::num::NonZeroU64;

use ferrastra_core::{
    AlphaMode, AuthoringDescriptor, CancellationContract, CapabilitySet, ComputationDescriptor,
    ConformanceProof, ConformanceRequirements, CostEstimate, EdgeMode, ExposureClass, InputDemand,
    IntRect, Locality, MemoryEstimate, MemoryEstimateError, Operation, OperationCategory,
    OperationContractError, OperationDamageRequest, OperationDescriptor, OperationIdentity,
...
const SEMANTIC_ID: &str = "ferrastra.source.raster";
const SEMANTIC_VERSION: u32 = 1;

/// Immutable raster source operation contract resolved by the runtime's source adapter.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct RasterSourceOperation {
    descriptor: OperationDescriptor,
}

impl RasterSourceOperation {
    /// Construct and validate the complete built-in source descriptor.
    ///
    /// # Errors
    ///
    ...
    pub fn new() -> Result<Self, OperationDefinitionError> {
        let descriptor = OperationDescriptor {
            ...
            ports: Box::new([PortDescriptor {
                ...
                product: ProductSpec::raster(RasterFormat::Rgba8PremultipliedEncoded),
                required: true,
            }]),
            parameters: Box::default(),
            computation: computation_descriptor(),
            authoring: AuthoringDescriptor {
                summary: "Immutable raster source".into(),
                details:
                    "Provides exact regional samples from one retained native source revision."
                        .into(),
                use_cases: Box::new(
                    ["Bind host-owned immutable raster content to a graph.".into()],
                ),
                warnings: Box::new([
...
impl Operation for RasterSourceOperation {
