...
//! Responsibility: Declare canonical bilinear affine sampling for RGBA8 rasters.
//!
//! Does not own: source storage, viewport policy, Qt adaptation, or scheduling.
...
use crate::{OperationDefinitionError, affine_bilinear_cpu};

const SEMANTIC_ID: &str = "ferrastra.resample.affine-bilinear";
const SEMANTIC_VERSION: u32 = 1;

/// Exact bilinear sampling through an explicit output-to-source affine transform.
#[derive(Clone, Debug)]
pub struct AffineBilinearOperation {
    descriptor: OperationDescriptor,
    source_port: PortId,
    colors: Arc<ColorPipeline>,
}

impl AffineBilinearOperation {
    /// Construct and validate the complete affine sampling descriptor.
    ///
    /// # Errors
    ///
    ...
    pub fn new() -> Result<Self, OperationDefinitionError> {
        let descriptor = OperationDescriptor {
            ...
            authoring: AuthoringDescriptor {
                summary: "Affine bilinear raster sampling".into(),
                details: "Samples premultiplied RGBA8 pixels through an explicit output-to-source affine transform with explicit transparent or clamped edges.".into(),
                use_cases: Box::new([
                    "Project raster fragments into explicit destination-coordinate storage."
                        .into(),
                ]),
                warnings: Box::new([
                    "Strong minification should use a reconstruction filter with declared antialiasing support."
                        .into(),
                ]),
            },
...
impl Operation for AffineBilinearOperation {
...
impl OperationKernel for AffineBilinearOperation {
    fn execute(
        &self,
        request: &OperationRequest,
        ...
        let working_space = working_space(&request.parameters)
            .map_err(|_| OperationExecutionError::InvalidProduct)?;
        let edge =
            edge_mode(&request.parameters).map_err(|_| OperationExecutionError::InvalidProduct)?;
        let evaluation = affine_bilinear_cpu::AffineBilinearEvaluation::new(
            contract,
            edge,
            working_space,
            budget,
        ...
        affine_bilinear_cpu::execute(request, &inputs[0], &mut outputs[0], &evaluation)
    }
}

fn working_space(
    parameters: &ferrastra_core::OperationParameters,
) -> Result<WorkingSpace, OperationContractError> {
    let Some(ParameterValue::Enum(value)) = parameters.get_named("working_space") else {
        return Err(OperationContractError::InvalidParameters);
    };
    match value.as_str() {
...
fn parameters() -> Result<Vec<ParameterDescriptor>, OperationDefinitionError> {
    ...
    values.push(ParameterDescriptor {
        id: ParameterId::new("working_space")?,
        parameter_type: ParameterType::Enum,
        unit: Unit::Unitless,
        default: ParameterValue::Enum(ParameterId::new("srgb_linear")?),
        hard_range: None,
        recommended_range: None,
        enum_values: ["srgb_encoded", "srgb_linear"]
            .into_iter()
            .map(ParameterId::new)
